...
export default function TeacherSubmissionsPanel(p: Props) {
  ...
  return <section>
    ...
    <div style={{ display: "grid", gap: 14, marginBottom: 18, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 16, padding: 14, boxShadow: "0 2px 8px rgba(15, 23, 42, 0.04)" }}>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em" }}>View</span><div style={{ display: "inline-flex", gap: 4, border: "1px solid #d7deea", borderRadius: 999, padding: 3, background: "#eef2f7" }}><button type="button" onClick={() => setViewMode("by_student")} style={{ border: "none", borderRadius: 999, padding: "7px 12px", fontSize: 12, fontWeight: 700, background: viewMode === "by_student" ? "#2563eb" : "transparent", color: viewMode === "by_student" ? "#ffffff" : "#475569", transition: "all 0.2s ease", cursor: "pointer" }}>By student</button><button type="button" onClick={() => setViewMode("by_submission")} style={{ border: "none", borderRadius: 999, padding: "7px 12px", fontSize: 12, fontWeight: 700, background: viewMode === "by_submission" ? "#2563eb" : "transparent", color: viewMode === "by_submission" ? "#ffffff" : "#475569", transition: "all 0.2s ease", cursor: "pointer" }}>By submission</button></div></div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em" }}>Status</span><div style={{ display: "inline-flex", gap: 4, border: "1px solid #d7deea", borderRadius: 999, padding: 3, background: "#eef2f7" }}>{(["all", "needs_review", "reviewed"] as const).map((filter) => <button key={filter} type="button" onClick={() => p.setReviewFilter(filter)} style={{ border: "none", borderRadius: 999, padding: "7px 12px", fontSize: 12, fontWeight: 700, background: p.reviewFilter === filter ? "#2563eb" : "transparent", color: p.reviewFilter === filter ? "#ffffff" : "#475569", transition: "all 0.2s ease", cursor: "pointer" }}>{filter === "all" ? "All" : filter === "needs_review" ? "Needs review" : "Reviewed"}</button>)}</div></div>
        <button type="button" onClick={p.onRefreshSubmissions} disabled={p.isLoadingSubmissions} style={{ marginLeft: "auto", border: "1px solid #cbd5e1", background: "#f8fafc", borderRadius: 999, padding: "8px 14px", fontSize: 12, fontWeight: 700, color: "#475569", cursor: "pointer" }}>{p.isLoadingSubmissions ? "Refreshing..." : "Refresh"}</button>
      </div>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        <label style={{ display: "grid", gap: 4, fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.04em" }}>Class
          <div style={{ borderRadius: 999, background: "#eef2f7", border: "1px solid #d7deea", padding: "0 10px" }}><select value={classFilter} onChange={(e) => setClassFilter(e.target.value)} style={{ minHeight: 36, borderRadius: 999, border: "none", background: "transparent", padding: "0 2px", fontSize: 13, color: "#0f172a", outline: "none" }}><option value="__all_classes__">All classes</option>{classOptions.map((c) => <option key={c} value={c}>{c}</option>)}</select></div>
        </label>
        <label style={{ display: "grid", gap: 4, fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.04em" }}>Assignment
          <div style={{ borderRadius: 999, background: "#eef2f7", border: "1px solid #d7deea", padding: "0 10px" }}><select value={p.submissionPromptFilter} onChange={(e) => p.setSubmissionPromptFilter(e.target.value)} style={{ minHeight: 36, borderRadius: 999, border: "none", background: "transparent", padding: "0 2px", fontSize: 13, color: "#0f172a", outline: "none" }}><option value="__all_prompts__">All assignments</option>{p.submissionPromptOptions.map((t) => <option key={t} value={t}>{t}</option>)}</select></div>
        </label>
      </div>
    </div>
    ...
    {viewMode === "by_submission" ? classFiltered.map((s) => renderSubmission(s)) : students.map((student) => {
      ...
      return <div key={student.key} style={{ border: "1px solid #dbe3f0", borderRadius: 16, padding: 16, marginBottom: 14, background: "#ffffff", boxShadow: "0 8px 20px rgba(15, 23, 42, 0.05)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
          <div><div style={{ fontWeight: 800, fontSize: 20, color: "#0f172a", letterSpacing: "0.01em" }}>{student.code}</div><div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 6 }}><span style={{ fontSize: 11, borderRadius: 999, border: "1px solid #d1d9e6", padding: "4px 10px", fontWeight: 700, color: "#475569", background: "#f8fafc" }}>{student.className}</span><span style={{ fontSize: 11, borderRadius: 999, border: "none", padding: "4px 10px", fontWeight: 700, color: status === "Needs review" ? "#92400e" : "#166534", background: status === "Needs review" ? "#fef3c7" : "#dcfce7" }}>{status}</span></div><div style={{ fontSize: 11, color: "#64748b", marginTop: 8 }}>{student.submissions.length} submissions · {student.needsReviewCount} need review · latest {student.latest?.created_at ? new Date(student.latest.created_at).toLocaleString() : "Unknown"}</div></div>
          <div><button type="button" onClick={() => setExpandedStudents((prev) => ({ ...prev, [student.key]: !expanded }))} style={{ border: "1px solid #d1d9e6", borderRadius: 999, padding: "8px 14px", fontSize: 12, fontWeight: 700, background: "#f8fafc", color: "#334155", cursor: "pointer" }}>{expanded ? "Collapse" : "Expand"}</button></div>
        </div>
